...
<head>
	...
	<title>医院医疗收费标准与物价部门制定价格标准对比清单</title>
...
<form action="#" enctype="multipart/form-data" method="post" target="upload-result" id="uploadForm" name="uploadForm">
    <fieldset id="fd3">
        ...
        <div class="fieldset-body">
            ...
            <div id="div1">
             <font color="red">注:医院医疗收费标准与物价部门制定价格标准对比清单以照片形式上传</font></br>
...
//加载数据
 var url = '<%=request.getContextPath()%>/work/f20010115/querySfdbInfo.action';
...
function search() {
			mini.open({
			    ...
			    title: "医院医疗收费标准与物价部门制定价格标准对比清单附件", width: 1000, height: 600,
			    onload: function () {
...
function saveData(){
		...
		var options  = {    
		url:'<%=request.getContextPath() %>/work/f20010115/saveSfdbInfo.action',   
			type:'post',    
