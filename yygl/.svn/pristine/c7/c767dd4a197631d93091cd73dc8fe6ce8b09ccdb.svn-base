...
<form action="#" enctype="multipart/form-data" method="post" target="upload-result" id="uploadForm" name="uploadForm">
    <fieldset id="fd3">
        ...
        <div class="fieldset-body">
            ...
            <div id="div1">
             <font color="red">注:设备清单（包括设备名称、数量、购进时间），设备发票复印件以照片形式上传</font></br>
...
//加载数据
 var url = '<%=request.getContextPath()%>/work/f20010111/querySbqdInfo.action';
...
function search() {
			mini.open({
			    ...
			    title: "设备清单附件", width: 1000, height: 600,
			    onload: function () {
...
function saveData(){
		...
		var options  = {    
		url:'<%=request.getContextPath() %>/work/f20010111/saveSbqdInfo.action',   
			type:'post',    
